...
if ( ! function_exists( 'ktpwp_supplier_cost_log' ) ) {
	/**
...
add_action(
    ...
    function () {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 権限エラー: ユーザーに権限がありません');
		...
		try {
			...
			if ( ! $table_exists ) {
				ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] テーブルが存在しません: ' . $table );
				wp_send_json( array() );
			}
			
			$query = "SELECT id, company_name FROM $table WHERE 1 ORDER BY company_name ASC";
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 実行クエリ: ' . $query );
			
			$suppliers = $wpdb->get_results( $query, ARRAY_A );
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] クエリ結果: ' . print_r( $suppliers, true ) );
			
			if ( $wpdb->last_error ) {
				ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] データベースエラー: ' . $wpdb->last_error );
				wp_send_json_error( __( 'データベースエラー: ', 'ktpwp' ) . $wpdb->last_error );
			}
			
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 協力会社リスト取得成功: ' . count( $suppliers ) . '件' );
			wp_send_json( $suppliers );
			
		} catch ( Exception $e ) {
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 例外発生: ' . $e->getMessage() );
...
add_action(
    ...
    function () {
		...
		if ( ! $supplier_id ) {
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] supplier_idが不正: ' . $supplier_id );
		...
		",
			...
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 実行クエリ: ' . $sql );
			
			$skills = $wpdb->get_results( $sql, ARRAY_A );
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] クエリ結果: ' . print_r( $skills, true ) );
			...
			ktpwp_supplier_cost_log('[SUPPLIER-COST-AJAX] 職能リスト取得成功: ' . count( $skills ) . '件' );
			wp_send_json( $skills );
...
add_action(
    ...
    function () {
		...
		if ( $has_supplier_id ) {
			...
			$format[] = '%d';
			ktpwp_supplier_cost_log( 'supplier_idを追加: ' . $data['supplier_id'] );
		}

		ktpwp_supplier_cost_log( 'Data to save: ' . print_r( $data, true ) );
		ktpwp_supplier_cost_log( 'Format: ' . print_r( $format, true ) );

		if ( $item_id > 0 ) {
			// UPDATE処理
			ktpwp_supplier_cost_log( '=== UPDATE処理開始 ===' );
			ktpwp_supplier_cost_log( 'UPDATE条件: id = ' . $item_id );

			$result = $wpdb->update( $table, $data, array( 'id' => $item_id ), $format, array( '%d' ) );
			ktpwp_supplier_cost_log( 'UPDATE result: ' . print_r( $result, true ) );
			ktpwp_supplier_cost_log( 'wpdb->last_error: ' . $wpdb->last_error );
			ktpwp_supplier_cost_log( 'wpdb->last_query: ' . $wpdb->last_query );

			if ( $result !== false && $result > 0 ) {
				ktpwp_supplier_cost_log( 'UPDATE成功: ID ' . $item_id );
				...
				ktpwp_supplier_cost_log( 'UPDATE失敗: 0行更新またはエラー' );
				// レコードの存在確認
				$existing_record = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM `{$table}` WHERE id = %d", $item_id ) );
				if ( ! $existing_record ) {
					ktpwp_supplier_cost_log( 'レコードが存在しないためINSERTを試行' );
					// INSERT処理にフォールバック
					$data['created_at'] = current_time( 'mysql' );
					$format[] = '%s';
					$result = $wpdb->insert( $table, $data, $format );
					if ( $result ) {
						ktpwp_supplier_cost_log( 'INSERT成功: 新規ID ' . $wpdb->insert_id );
						wp_send_json_success(
                            array(
								'id' => $wpdb->insert_id,
						...
						ktpwp_supplier_cost_log( 'INSERT失敗: ' . $wpdb->last_error );
						wp_send_json_error( __( '新規追加失敗: ', 'ktpwp' ) . $wpdb->last_error );
					}
				} else {
					ktpwp_supplier_cost_log( 'レコードは存在するが更新条件に合致しない' );
					wp_send_json_error( __( '更新失敗: レコードは存在するが更新条件に合致しません', 'ktpwp' ) );
				}
			}
		} else {
			// INSERT処理
			ktpwp_supplier_cost_log( '=== INSERT処理開始 ===' );
			if ( $force_save || ! empty( $product_name ) ) {
				$data['created_at'] = current_time( 'mysql' );
				$format[] = '%s';
				ktpwp_supplier_cost_log( 'INSERT実行' );
				ktpwp_supplier_cost_log( 'INSERT data: ' . print_r( $data, true ) );

				$result = $wpdb->insert( $table, $data, $format );
				ktpwp_supplier_cost_log( 'INSERT result: ' . print_r( $result, true ) );
				ktpwp_supplier_cost_log( 'wpdb->last_error: ' . $wpdb->last_error );
				ktpwp_supplier_cost_log( 'wpdb->last_query: ' . $wpdb->last_query );
				ktpwp_supplier_cost_log( 'wpdb->insert_id: ' . $wpdb->insert_id );
				...
				ktpwp_supplier_cost_log( 'INSERT失敗: 商品名が空' );
				wp_send_json_error( __( '商品名が空のため追加されません', 'ktpwp' ) );
			}
		}

		ktpwp_supplier_cost_log( '=== ktpwp_save_order_cost_item 終了 ===' );
	}
);
