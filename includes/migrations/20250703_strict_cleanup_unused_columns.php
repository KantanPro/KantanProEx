 ...
 * 厳密な不要カラム削除マイグレーション
 * 実行日: 2025-07-03
 * 目的: 実際に使用されていないカラムのみを残してテーブル構造を大幅に最適化
 */
...
if ( class_exists( 'KTPWP_Fresh_Install_Detector' ) ) {
    ...
    if ( $fresh_detector->should_skip_migrations() ) {
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            ktpwp_debug_log( 'KTPWP Migration: 新規インストール環境のため20250703_strict_cleanup_unused_columnsをスキップ' );
        }
        return;
    }
}
...
if ( $migration_completed ) {
    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
        ktpwp_debug_log( 'KTPWP Migration: 厳密なカラムクリーンアップは既に実行済みです' );
    }
    return;
}
...
// 最終結果のログ出力
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    ktpwp_debug_log( 'KTPWP Migration: 厳密なカラムクリーンアップ完了' );
    ...
    ktpwp_debug_log( 'KTPWP Migration: 最終ORDERカラム: ' . implode( ', ', $final_order_columns ) );
    ktpwp_debug_log( 'KTPWP Migration: 最終SUPPLIERカラム数: ' . count( $final_supplier_columns ) );
    ktpwp_debug_log( 'KTPWP Migration: 最終SUPPLIERカラム: ' . implode( ', ', $final_supplier_columns ) );
}

// マイグレーション完了フラグを設定
update_option( $migration_key, true );
update_option( 'ktp_strict_cleanup_timestamp', current_time( 'mysql' ) );

if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    ktpwp_debug_log( 'KTPWP Migration: 20250703_strict_cleanup_unused_columns 完了' );
}
