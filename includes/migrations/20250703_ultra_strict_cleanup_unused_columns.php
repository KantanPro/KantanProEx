...
if ( class_exists( 'KTPWP_Fresh_Install_Detector' ) ) {
    ...
    if ( $fresh_detector->should_skip_migrations() ) {
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            ktpwp_debug_log( 'KTPWP Migration: 新規インストール環境のため20250703_ultra_strict_cleanup_unused_columnsをスキップ' );
        }
        return;
    }
}
...
if ( $migration_completed ) {
    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
        ktpwp_debug_log( 'KTPWP Migration: 超厳密なカラムクリーンアップは既に実行済みです' );
    }
    return;
}
...
// 最終結果のログ出力
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    ktpwp_debug_log( 'KTPWP Migration: 超厳密カラムクリーンアップ完了' );
...
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    ktpwp_debug_log( 'KTPWP Migration: 20250703_ultra_strict_cleanup_unused_columns 完了' );
}
