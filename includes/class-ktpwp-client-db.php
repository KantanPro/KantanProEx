	...
	class KTPWP_Client_DB {
		...
		public function update_table( $tab_name ) {
			...
			if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					...
					ktpwp_debug_log( 'KTPWP Client Debug: update_table - REQUEST_METHOD = ' . $_SERVER['REQUEST_METHOD'] );
					ktpwp_debug_log( 'KTPWP Client Debug: update_table - POST data keys = ' . implode( ', ', array_keys( $_POST ) ) );
				}
		...
		private function sanitize_post_data( $post_data ) {
			...
			return array(
				...
				'payment_timing' => isset( $post_data['payment_timing'] ) && in_array( $post_data['payment_timing'], array( 'postpay', 'prepay', 'prepay_wc' ), true ) ? $post_data['payment_timing'] : 'postpay',
		...
		private function handle_insert( $table_name, $tab_name, $fields_data, $search_field_value ) {
			...
			$result = $wpdb->insert(
				...
				array(
					...
					'payment_timing' => isset( $fields_data['payment_timing'] ) && in_array( $fields_data['payment_timing'], array( 'postpay', 'prepay', 'prepay_wc' ), true ) ? $fields_data['payment_timing'] : 'postpay',
