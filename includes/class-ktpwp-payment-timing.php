...
class KTPWP_Payment_Timing {
	 ...
	 *
	 * @param object|null $order 受注オブジェクト。
	 * @return bool
	 */
	public static function is_public_web_order( $order ) {
		if ( ! $order || ! is_object( $order ) ) {
			return false;
		}

		$external = isset( $order->external_source ) ? trim( (string) $order->external_source ) : '';
		if ( $external === 'public_product' ) {
			return true;
		}

		if ( class_exists( 'KTPWP_Public_Product_Order_Memo' ) ) {
			return KTPWP_Public_Product_Order_Memo::is_web_application( $order->memo ?? '' );
		}
	 ...
	 * @return string '' | 'WEB受注' | '前入金済' | 'EC受注' | 'WC受注'
	 */
	public static function get_inbound_source_label_for_order_id( $order_id ) {
		...
		$order        = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT o.id, o.payment_timing, o.external_source, o.client_id, o.memo FROM `{$order_table}` o WHERE o.id = %d",
				$order_id
			)
		);
		if ( ! $order ) {
		...
		return self::get_inbound_source_label( $order, $client );
	}
