...
* スタッフチャット送信のセキュリティ検証を厳格化。従来は権限があれば検証を省略できる状態でしたが、常に検証するよう修正しました
* デバッグログの出力方法を見直し。顧客名や金額を含む送信内容がサーバーのログへ常時記録されていた問題を解消し、デバッグ時以外はログを出力しないようにしました
* 仕事リストの表示を高速化。受注書テーブルに検索・並び替え用のインデックスを追加し、一覧表示のたびに顧客情報を1件ずつ問い合わせていた処理を一括取得に変更しました。表示内容は変わりません
* 受注書テーブルの定義が3種類に分かれ、環境によっては受注書の登録に失敗しうる問題を修正。定義を1つに統一しました
* 配布ファイルのサイズを削減。JavaScript と CSS を配布時に圧縮するようにし、画面の読み込みを軽くしました（動作・見た目は変わりません）
...
= 1.4.24 - 2026年08月03日 =
* Contact Form 7経由の受注取り込みにスパム対策を追加。全CF7フォームへハニーポット欄・送信タイムトラップを自動付与し、HTMLタグ混入や過剰なURL記載を含む送信を受注登録前にブロック
...
= 1.4.17 - 2026年07月20日 =
* 既存顧客の CF7 問い合わせで送信が完了しない不具合を修正（is_placeholder_company_name 未定義による Fatal Error）
...
* 一括請求プレビューでロゴ・印影をドラッグしてサイズ調整できる機能を追加（管理者のみ・保存対応）
...
= 1.4.7 - 2026年06月28日 =
* スタッフチャットに「更新」ボタンを追加（送信ボタン横・ページ再読み込みで最新メッセージを表示）
...
= 1.4.5 - 2026年06月26日 =
* プラグイン更新後も「新しいバージョンが利用可能」と表示され続ける不具合を修正
* 保存済み更新情報をインストール版と照合し、古いキャッシュを自動で正規化
...
= 1.4.2 - 2026年06月26日 =
* ブロック中顧客の案件を仕事リスト・受注書タブから非表示に変更
* Contact Form 7 からの問い合わせもブロック対象に追加
