...
(function($) {
    ...
    const SVG_ICONS = {
        ...
        'cached': '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>',
        ...
        'contact_mail': '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h4l4 4 4-4h4c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-7 10H6v-1.5h7V14zm3-3H6V9.5h10V11z"/></svg>',
        ...
        'edit': '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>',
        ...
        'do_not_disturb_off': '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4c4.41 0 7.96 3.56 7.96 7.97 0 .55-.06 1.09-.17 1.61L16.6 9.21C16.06 7.31 14.21 6 12 6c-3.31 0-6 2.69-6 6 0 2.21 1.2 4.14 2.98 5.17L4.71 9.9C4.25 10.56 4 11.27 4 12c0 4.42 3.58 8 8 8 1.41 0 2.73-.37 3.87-1.02l1.49 1.49C16.26 21.19 14.2 22 12 22 6.48 22 2 17.52 2 12S6.48 2 12 2c2.75 0 5.27 1.07 7.17 2.81L12.71 4.29C12.48 4.1 12.25 4 12 4zm5.17 3.99L7 18.16C8.28 19.22 10.04 20 12 20c4.41 0 8-3.59 8-8 0-1.48-.4-2.87-1.11-4.06L17.17 7.99z"/></svg>',
        'arrow_upward': '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M4 12l1.41 1.41L11 7.83V20h2V7.83l5.58 5.59L20 12l-8-8-8 8z"/></svg>',
    };
    ...
    function getSvgIcon(iconName, attributes = {}) {
        ...
        return `<span${attrString}>${SVG_ICONS[iconName]}</span>`;
    }

    // Material SymbolsクラスをSVGアイコンに置換する関数
    function replaceMaterialSymbols(html) {
        const patterns = [
        ...
        patterns.forEach(pattern => {
            result = result.replace(pattern, (match, iconName) => {
                ...
                return getSvgIcon(trimmedIconName, attributes);
            });
        });
    ...
    window.KTPSvgIcons = {
        getIcon: getSvgIcon,
        replaceMaterialSymbols: replaceMaterialSymbols
    };

    // jQueryプラグインとしても提供
    $.fn.svgIcon = function(iconName, attributes = {}) {
        return this.each(function() {
            $(this).html(getSvgIcon(iconName, attributes));
        });
    };

    // ページ読み込み時に既存のMaterial Symbolsを置換
    $(document).ready(function() {
        // 既存のMaterial SymbolsをSVGアイコンに置換
        $('span.material-symbols-outlined').each(function() {
            const $span = $(this);
            const iconName = $span.text().trim();
            ...
            $span.replaceWith(getSvgIcon(iconName, attributes));
        });
    });
